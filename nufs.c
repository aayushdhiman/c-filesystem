...
int nufs_readdir(const char *path, void *buf, fuse_fill_dir_t filler,
  ...
  // path list get
  slist_t* fullpath = storage_list(path);
  if (!fullpath)
  {
    // end clause
    printf("readdir(%s) -> %d\n", path, rv);
  ...
  // make mutable
  slist_t* pathlist = fullpath;

  int path_len = strlen(path);
  while (pathlist)
  {
    // iterate, malloc, concatenate, free, repeat
    char* path_iter = (char*)malloc(path_len + 50);
    strncpy(path_iter, path, path_len);
    if (path[path_len - 1]  == '/')
      path_iter[path_len] = '\0';
    else
    {
      path_iter[path_len] = '/';
      path_iter[path_len + 1] = '\0';
    }
    strncat(path_iter, pathlist->data, DIR_NAME_LENGTH);
    nufs_getattr(path_iter, &st);
    filler(buf, pathlist->data, &st, 0);
...
//represents the mkdir system call
...
//represents the unlink system call
...
//represents the link system call
...
//represents the rmdir system call
...
int nufs_chmod(const char *path, mode_t mode) {
  ...
  // chmod
  get_inode(rv)->mode &= ~07777 & mode;
  
  printf("chmod(%s, %04o) -> %d\n", path, mode, rv);
  return rv;
}

//truncates the size of a file to a length
...
//initializes all operations of nufs
